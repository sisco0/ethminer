...
struct VulkanChannel : public LogChannel
{
    static const char* name() { return EthOrange "vk"; }
    static const int verbosity = 2;
    static const bool debug = false;
};
#define vklog clog(VulkanChannel)
...
VulkanMiner::VulkanMiner(unsigned _index, VulkanSettings _settings, DeviceDescriptor& _device)
  : Miner("vk-", _index), m_settings(_settings)
{
...
VulkanMiner::~VulkanMiner()
{
    DEV_BUILD_LOG_PROGRAMFLOW(vklog, "cl-" << m_index << " VulkanMiner::~VulkanMiner() begin");
    stopWorking();
    kick_miner();
    DEV_BUILD_LOG_PROGRAMFLOW(vklog, "cl-" << m_index << " VulkanMiner::~VulkanMiner() end");
}
            ...
            if (results.count)
            {
                ...
                for (uint32_t i = 0; i < results.count; i++)
                {
                    ...
                    if (nonce != m_lastNonce)
                    {
                        ...
                        vklog << EthWhite << "Job: " << current.header.abridged() << " Sol: 0x"
...
void VulkanMiner::enumDevices(std::map<string, DeviceDescriptor>& _DevicesCollection)
{
...
bool VulkanMiner::initDevice()
{
    ...
    else
    {
        ...
        vklog << "Unrecognized Platform";
        return false;
    }
    if (!m_settings.noExit && (m_hwmoninfo.deviceType != HwMonitorInfoType::AMD))
    {
        m_settings.noExit = true;
        vklog << "no exit option enabled for non AMD opencl device";
    }
            ...
            m_deviceDescriptor.clPlatformVersionMinor == 1))
    {
        ...
        {
            vklog << "OpenCL " << m_deviceDescriptor.clPlatformVersion
...
bool VulkanMiner::initEpoch_internal()
{
    ...
    {
        vklog << "Epoch " << m_epochContext.epochNumber << " requires "
    ...
    vklog << "Generating split DAG + Light (total): "
    ...
    try
    {
        ...
        if (!m_settings.noBinary)
        {
            ...
            vklog << "Loading binary kernel " << fname_strm.str();
            try
            {
        ...
        try
        {
            vklog << "Creating DAG buffer, size: "
            ...
            vklog << "Creating light cache buffer, size: "
            ...
            if (light_on_host)
            {
                ...
                vklog << "WARNING: Generating DAG will take minutes, not seconds";
            }
            vklog << "Loading kernels";
        ...
        vklog << "Creating buffer for header.";
        m_header.clear();
        ...
        vklog << dev::getFormattedMemory((double)m_epochContext.dagSize)
              << " of DAG data generated in "
              << dagTime.count() << " ms.";
    }
    catch (cl::Error const& err)
    {
        vklog << ethCLErrorHelper("OpenCL init failed", err);
        pause(MinerPauseEnum::PauseDueToInitEpochError);
        return false;
    }
    return true;
